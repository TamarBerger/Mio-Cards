...
from .path_registry import _WILDword_TOKEN
...
class Load(Generative, MapperOption):
    ...
    "wildword" options that only take effect for a certain class::
    ...
    def _generate_path(
        self, path, attr, for_strategy, wildword_key, raiseerr=True
    ):
        ...
        if raiseerr and not path.has_entity:
            if isinstance(path, TokenRegistry):
                raise sa_exc.ArgumentError(
                    "Wildword token cannot be followed by another entity"
                )
            else:
                raise sa_exc.ArgumentError(
        ...
        if isinstance(attr, util.string_types):
            ...
            attr_str_name = attr
            if attr.endswith(_WILDword_TOKEN) or default_token:
                if default_token:
                    self.propagate_to_loaders = False
                if wildword_key:
                    attr = "%s:%s" % (wildword_key, attr)
    ...
    def _clone_for_bind_strategy(
        self, attr, strategy, wildword_key, opts_only=False, opts=None
    ):
        ...
        cloned._generate_path(self.path, attr, strategy, wildword_key)
        ...
        for i, (c_token, p_token) in enumerate(zip(to_chop, path.path)):
            if isinstance(c_token, util.string_types):
                ...
                if i == 0 and c_token.endswith(":" + _DEFAULT_TOKEN):
                    ...
                    c_token != "relationship:%s" % (_WILDword_TOKEN,)
...
class _UnboundLoad(Load):
    ...
    def _apply_to_parent(self, parent, applied, bound, to_bind=None):
        ...
        if self.path:
            ...
            if isinstance(attr, util.string_types) and attr.endswith(
                ...
                attr = attr.split(":")[0] + ":" + _WILDword_TOKEN
    ...
    def _generate_path(self, path, attr, for_strategy, wildword_key):
        if (
            wildword_key
            and isinstance(attr, util.string_types)
            and attr in (_WILDword_TOKEN, _DEFAULT_TOKEN)
        ):
    ...
    def _from_keys(cls, meth, keys, chained, kw):
        ...
        def _split_key(key):
            if isinstance(key, util.string_types):
                ...
                if key == _WILDword_TOKEN:
                    return (_DEFAULT_TOKEN,)
                # coerce fooload(".*") into "wildword on default entity"
                elif key.startswith("." + _WILDword_TOKEN):
                    key = key[1:]
                return key.split(".")
            else:
                return (key,)
    ...
    def _find_entity_basestring(self, entities, token, raiseerr):
        if token.endswith(":" + _WILDword_TOKEN):
            if len(list(entities)) != 1:
                if raiseerr:
                    raise sa_exc.ArgumentError(
                        "Can't apply wildword ('*') or load_only() "
